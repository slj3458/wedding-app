...
const PhotoGallery = ({ isAdmin = false, adminToken = null }) => {
  ...
  useEffect(() => {
    if (messages.length > 0) {
      ...
      } else if (latestMessage.type === 'photo_deleted' || latestMessage.type === 'photo_hidden') {
        // Remove photo from display when admin deletes/hides it
        setPhotos(prev => prev.filter(photo => photo.id !== latestMessage.data.id));
      }
    }
  }, [messages]);
  ...
  useEffect(() => {
    ...
    if (searchTerm.trim()) {
      ...
      result = result.filter(photo =>
        ...
        (photo.uploaded_by && photo.uploaded_by.toLowerCase().includes(search))
      );
    }
...
// Function to fetch photos from backend
  ...
  const likePhoto = async (photoId) => {
    try {
      await fetch(`http://localhost:8000/api/gallery/photo/${photoId}/like`, {
        method: 'POST'
      });
    } catch (error) {
  ...
  const downloadPhoto = async (filename, originalFilename) => {
    try {
      const response = await fetch(`http://localhost:8000/api/gallery/photo/${filename}`);
        ...
        {filteredPhotos.map(photo => (
          <div key={photo.id} style={styles.photoCard}>
            ...
            <div style={styles.imageContainer}>
              <img
                src={`http://localhost:8000/api/gallery/thumbnail/${photo.filename}`}
                alt={photo.caption || 'Wedding photo'}
                style={styles.image}
                onClick={() => window.open(`http://localhost:8000/api/gallery/photo/${photo.filename}`, '_blank')}
              />
