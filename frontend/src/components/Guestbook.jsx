...
const Guestbook = ({ isAdmin = false, adminToken = null }) => {
  ...
  useEffect(() => {
    if (messages.length > 0) {
      ...
      } else if (latestMessage.type === 'guestbook_deleted' || latestMessage.type === 'guestbook_hidden') {
        // Remove entry from display when admin deletes/hides it
        setEntries(prev => prev.filter(entry => entry.id !== latestMessage.data.id));
      }
    }
  }, [messages]);
  ...
  const fetchEntries = async () => {
    try {
      const response = await fetch('http://localhost:8000/api/guestbook/entries');
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch('http://localhost:8000/api/guestbook/entry', {
        method: 'POST',
        headers: {
      ...
      setNotification({ type: 'success', text: '✓ Your message has been added!' });
      setName('');
      setMessage('');
      ...
      setNotification({ type: 'error', text: 'Failed to submit. Please try again.' });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div style={styles.container}>
      ...
      <p style={styles.subtitle}>Leave your wishes and memories for the happy couple</p>
